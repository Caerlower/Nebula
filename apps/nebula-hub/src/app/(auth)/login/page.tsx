...
import { applyPrivySession, syncHubSession } from "@/lib/auth/session";
...
function LoginForm() {
  ...
  const goToApp = useRef(() => {
    ...
    void syncHubSession().catch((error) => {
      console.error("[login] hub session sync", error);
    });
  ...
  // Stable callbacks for Privy hooks (recreating them mid-OAuth breaks the flow).
  const onAuthComplete = useRef(() => {
    const privyUser = user;
    if (!privyUser || !hydrated) return;
    applyPrivySession(privyUser, { onboarded: true });
    goToApp.current();
  });
  ...
  const { sendCode, loginWithCode, state: emailState } = useLoginWithEmail({
    ...
    onError: (error) => {
      ...
      toast.error("Email sign-in failed", { description: String(error) });
    },
  });
  const { initOAuth, state: oauthState } = useLoginWithOAuth({
    ...
    onError: (error) => {
      setOauthBusy(null);
      redirected.current = false;
      toast.error("OAuth sign-in failed", { description: String(error) });
    },
  });
  ...
  useEffect(() => {
    ...
    applyPrivySession(user, { onboarded: true });
    goToApp.current();
  }, [ready, hydrated, authenticated, user]);

  // OAuth return that never authenticates.
  useEffect(() => {
  ...
  const submit = async (values: Values) => {
    ...
    try {
      ...
      toast.error("Sign-in failed", {
        description: "Check the code from your email.",
      });
    }
  };
  ...
  const oauthLoading = oauthBusy !== null;
      ...
      <AuthSplash
        ...
        detail={
          !hydrated || !ready
            ...
            : "Finishing up — this only takes a moment."
        }
      />
    );
  }
